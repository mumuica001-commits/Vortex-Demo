...
/**
 * True only when o operador configurou credenciais reais do broker social
 * (ver `VITE_SOCIAL_LOGIN_ENABLED` + `GROK_AUTH_CLIENT_ID/SECRET` no server).
 * Fora da plataforma Grok isso normalmente fica `false` — os botões de
 * Google/X ficam ocultos e sobra só email/senha, que sempre funciona.
 */
export const socialLoginEnabled = import.meta.env.VITE_SOCIAL_LOGIN_ENABLED === "true";
