...
const grokAuthClientIdExplicit = env("GROK_AUTH_CLIENT_ID");
const grokAuthClientSecretExplicit = env("GROK_AUTH_CLIENT_SECRET");
const grokClientId = grokAuthClientIdExplicit ?? PREVIEW_CLIENT_ID;
const grokClientSecret = grokAuthClientSecretExplicit ?? PREVIEW_CLIENT_SECRET;
...
// SEGURANÇA (autohospedagem fora da plataforma Grok): o plugin de OAuth só é
// registrado com credenciais REAIS do broker (GROK_AUTH_CLIENT_ID/SECRET
// injetadas explicitamente). `authConfigured` teria fallback para as
// constantes de preview (`PREVIEW_CLIENT_ID`), que o broker real só aceita
// para hosts `*.grok-sandbox.com` — noutro domínio o botão apareceria mas
// falharia sempre. Sem essas credenciais, os botões de Google/X ficam
// ocultos no client (ver `client.ts`) e login funciona só por email/senha.
const socialLoginConfigured = Boolean(grokAuthClientIdExplicit && grokAuthClientSecretExplicit);

// Built separately so the `betterAuth({...})` call stays easy to edit without
// breaking brackets (models often trip on the conditional plugin spread).
const grokOAuthPlugin = socialLoginConfigured
  ? genericOAuth({
